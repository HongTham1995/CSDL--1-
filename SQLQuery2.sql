...
BEGIN
    BEGIN TRY
        -- Debug thông báo
        PRINT 'Trigger trg_UpdateCauHoi_DLTS thực thi';

        -- Chèn mô tả mới vào CSDLNC nếu chưa tồn tại
        INSERT INTO CSDLNC.dbo.Mota (maMT, De_tai, Nguon, Link, Thoigian)
        SELECT 
            i.maMT, m.De_tai, m.Nguon, null, m.Thoigian
        FROM inserted i
        LEFT JOIN CSDLNC.dbo.Mota mt ON i.maMT = mt.maMT
        INNER JOIN Mota m ON i.maMT = m.maMT
        WHERE mt.maMT IS NULL;

        -- Sử dụng MERGE để cập nhật hoặc thêm mới câu hỏi
        MERGE CSDLNC.dbo.Cau_hoi AS target
        USING (SELECT * FROM inserted WHERE Nguoi_kiem_duyet_1 IS NOT NULL AND Nguoi_kiem_duyet_2 IS NOT NULL) AS source
        ON target.ID = source.ID
        WHEN MATCHED THEN
            UPDATE SET 
                cau_hoi = source.cau_hoi,
                ...
                dap_an_d = source.dap_an_d,
                dap_an_dung = source.dap_an_dung,
                Nguoi_kiem_duyet_1 = source.Nguoi_kiem_duyet_1,
                Nguoi_kiem_duyet_2 = source.Nguoi_kiem_duyet_2,
                maMT = source.maMT
        WHEN NOT MATCHED THEN
            INSERT (ID, cau_hoi, dap_an_a, dap_an_b, dap_an_c, dap_an_d, dap_an_dung, Nguoi_kiem_duyet_1, Nguoi_kiem_duyet_2, maMT)
            VALUES (source.ID, source.cau_hoi, source.dap_an_a, source.dap_an_b, source.dap_an_c, source.dap_an_d, source.dap_an_dung, source.Nguoi_kiem_duyet_1, source.Nguoi_kiem_duyet_2, source.maMT);

        -- Xóa câu hỏi nếu một trong hai người kiểm duyệt là NULL
        DELETE FROM CSDLNC.dbo.Cau_hoi
        WHERE ID IN (
            SELECT ID
            FROM inserted
            WHERE Nguoi_kiem_duyet_1 IS NULL OR Nguoi_kiem_duyet_2 IS NULL
        );
